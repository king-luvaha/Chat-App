        ...
        <div className='w-full'>
            ...
            <div className='h-[calc(100vh-65px)] overflow-x-hidden overflow-y-auto scrollbar'>
            {
                allUser.length === 0 && (
                    <div className='mt-12'>
                        ...
                        <p className='text-lg text-center text-slate-400'>Click on user to start a conversation.</p>    
                    </div>
                )
            }

            {
                allUser.map((conv,index)=>{
                    return(
                        <NavLink to={"/"+conv?.userDetails?._id} key={conv?._id} className='flex items-center gap-2 py-3 px-2 border border-transparent hover:bg-slate-100 cursor-pointer'>
                            <div>
                                <Avatar
